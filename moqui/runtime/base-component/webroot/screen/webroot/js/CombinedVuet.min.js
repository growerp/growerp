...
moqui.isArray(f)&&(d.navMenuList=f)}});window.history.pushState(null,this.ScreenTitle,a);this.urlListeners.forEach(function(e){e(a,this)},this);document.documentElement.scrollTop=0;document.body.scrollTop=0}}},setParameters:function(a){a&&(this.$root.currentParameters=$.extend({},this.$root.currentParameters,a));this.$root.reloadSubscreens()},addSubscreen:function(a){a.pathIndex=this.activeSubscreens.length;a.loadActive();this.activeSubscreens.push(a)},reloadSubscreens:function(){var a=this.currentPathList,
...
b},reloadContainer:function(a){var b=this.activeContainers[a];b?b.reload():console.error("Container with ID "+a+" not found, not reloading")},loadContainer:function(a,b){var c=this.activeContainers[a];c?c.load(b):console.error("Container with ID "+a+" not found, not loading url "+b)},addNavPlugin:function(a){var b=this;moqui.loadComponent(this.appRootPath+a,function(c){b.navPlugins.push(c)})},addNavPluginsWait:function(a,b){if(a&&a.length>b){this.addNavPlugin(a[b]);var c=this;a.length>b+1&&setTimeout(function(){c.addNavPluginsWait(a,
b+1)},500)}},addUrlListener:function(a){0<=this.urlListeners.indexOf(a)||this.urlListeners.push(a)},addNotify:function(a,b){var c=this.notifyHistoryList.slice(0),d=new Date,e=d.getHours();10>e&&(e="0"+e);d=d.getMinutes();10>d&&(d="0"+d);for(c.unshift({message:a,type:b,time:e+":"+d});25<c.length;)c.pop();this.notifyHistoryList=c},switchDarkLight:function(){var a=$("body");a.toggleClass("bg-dark");a.toggleClass("bg-light");a=a.hasClass("bg-dark")?"bg-dark":"bg-light";$.ajax({type:"POST",url:this.appRootPath+
"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"OUTER_STYLE",preferenceValue:a}})},showScreenDocDialog:function(a){$("#screen-document-dialog").modal("show");$("#screen-document-dialog-body").load(this.currentPath+"/screenDoc?docIndex\x3d"+a)},stopProp:function(a){a.stopPropagation()},getNavHref:function(a){a||(a=this.navMenuList.length-1);a=this.navMenuList[a];if(a.extraPathList&&a.extraPathList.length){var b=a.path+"/"+a.extraPathList.join("/"),
c=a.pathWithParams.indexOf("?");0<c&&(b+=a.pathWithParams.slice(c));return b}return a.pathWithParams||a.path},getLinkPath:function(a){this.appRootPath&&this.appRootPath.length&&0!==a.indexOf(this.appRootPath)&&(a=this.appRootPath+a);a=a.split("/");var b=this.appRootPath.split("/").length;a[b]=this.linkBasePath.split("/").slice(-1);return a=a.join("/")}},watch:{navMenuList:function(a){if(0<a.length){var b=a[a.length-1];a=1<a.length?a[a.length-2]:null;b.extraPathList&&(this.extraPathList=b.extraPathList);
var c=this.basePathSize,d=b.path.split("/").slice(c+1);console.info("nav updated fullPath "+JSON.stringify(d)+" currentPathList "+JSON.stringify(this.currentPathList)+" cur.path "+b.path+" basePathSize "+c);this.currentPathList=d;this.reloadSubscreens();a=(a?a.title+" - ":"")+b.title;c=b.pathWithParams;var e=c.indexOf("?");if(0<e){d=["pageIndex","orderBySelect","orderByField","moquiSessionToken"];var f=c.substring(e+1).split("\x26");c=c.substring(0,e);e=0;for(var g="",k=0;k<f.length;k++){var h=f[k];
c=-1===c.indexOf("?")?c+"?":c+"\x26";c+=h;if(!(3<e)){var l=h.indexOf("\x3d");if(0<l){var m=h.substring(0,l);h=h.substring(l+1);0<m.indexOf("_op")||0<m.indexOf("_not")||0<m.indexOf("_ic")||0<d.indexOf(m)||m===h||(0<g.length&&(g+=", "),g+=decodeURIComponent(h),e++)}}}0<g.length&&(70<g.length&&(g=g.substring(0,70)+"..."),a=a+" ("+g+")")}d=this.navHistoryList;for(f=0;f<d.length;)d[f].pathWithParams===c?d.splice(f,1):f++;for(d.unshift({title:a,pathWithParams:c,image:b.image,imageType:b.imageType});25<
d.length;)d.pop();document.title=a}},currentPathList:function(a){(a=a[a.length-1])&&$(this.$el).removeClass().addClass(a)}},computed:{currentPath:{get:function(){var a=this.currentPathList,b=this.extraPathList;return this.basePath+(a&&0<a.length?"/"+a.join("/"):"")+(b&&0<b.length?"/"+b.join("/"):"")},set:function(a){a&&0!==a.length?("/"===a.slice(a.length-1)&&(a=a.slice(0,a.length-1)),0===a.indexOf(this.linkBasePath)?a=a.slice(this.linkBasePath.length+1):0===a.indexOf(this.basePath)&&(a=a.slice(this.basePath.length+
1)),this.currentPathList=a.split("/")):this.currentPathList=[]}},currentLinkPath:function(){var a=this.currentPathList,b=this.extraPathList;return this.linkBasePath+(a&&0<a.length?"/"+a.join("/"):"")+(b&&0<b.length?"/"+b.join("/"):"")},currentSearch:{get:function(){return moqui.objToSearch(this.currentParameters)},set:function(a){this.currentParameters=moqui.searchToObj(a)}},currentLinkUrl:function(){var a=this.currentSearch;return this.currentLinkPath+(0<a.length?"?"+a:"")},basePathSize:function(){return this.basePath.split("/").length-
this.appRootPath.split("/").length},ScreenTitle:function(){return 0<this.navMenuList.length?this.navMenuList[this.navMenuList.length-1].title:""},documentMenuList:function(){for(var a=[],b=0;b<this.navMenuList.length;b++){var c=this.navMenuList[b].screenDocList;c&&c.length&&c.forEach(function(d){a.push(d)})}return a}},created:function(){this.moquiSessionToken=$("#confMoquiSessionToken").val();this.appHost=$("#confAppHost").val();this.appRootPath=$("#confAppRootPath").val();this.basePath=$("#confBasePath").val();
this.linkBasePath=$("#confLinkBasePath").val();this.userId=$("#confUserId").val();this.locale=$("#confLocale").val();moqui.localeMap[this.locale]&&(this.locale=moqui.localeMap[this.locale]);var a=$("#confOuterStyle").val();if(a){var b=$("body"),c=b.hasClass("bg-dark")?"bg-dark":"bg-light";c!==a&&(b.removeClass(c),b.addClass(a))}this.notificationClient=new moqui.NotificationClient(("https:"===location.protocol?"wss://":"ws://")+this.appHost+this.appRootPath+"/notws");var d=[];$(".confNavPluginUrl").each(function(e,
f){d.push($(f).val())});this.addNavPluginsWait(d,0)},mounted:function(){var a=$(this.$el);a.find('.navbar [data-toggle\x3d"tooltip"]').tooltip({placement:"bottom",trigger:"hover"});a.find("#history-menu-link").tooltip({placement:"bottom",trigger:"hover"});a.find("#notify-history-menu-link").tooltip({placement:"bottom",trigger:"hover"});a.find("#document-menu-link").tooltip({placement:"bottom",trigger:"hover"});this.setUrl(window.location.pathname+window.location.search);this.notificationClient.registerListener("ALL");
$("#screen-document-dialog").on("hidden.bs.modal",function(){var b=$("#screen-document-dialog-body");b.empty();b.append('\x3cdiv class\x3d"spinner"\x3e\x3cdiv\x3eLoading…\x3c/div\x3e\x3c/div\x3e')});window.Notification&&"granted"!==Notification.permission&&"denied"!==Notification.permission&&Notification.requestPermission(function(b){"granted"===b?moqui.notifyMessages("Browser notifications enabled, if you don't want them use browser notification settings to block"):"denied"===b&&moqui.notifyMessages("Browser notifications disabled, if you want them use browser notification settings to allow")})}});
