...
moqui.loadComponent=function(a,b,c){var d=moqui.urlExtensions.js,e=moqui.urlExtensions.vue,f=moqui.urlExtensions.vuet;if("string"===typeof a){var g=a.indexOf("?");if(0<g){var h=a.slice(0,g);var n=a.slice(g+1)}else h=a}else{h=a.path;var k=a.extraPath;n=a.search;var l=a.bodyParameters}console.log("Load Component "+JSON.stringify(a)+" Window Width "+window.innerWidth+" Quasar Platform: "+JSON.stringify(Quasar.Platform.is)+" search: "+n);!(600>=window.innerWidth||Quasar.Platform.is.mobile)||n&&-1!==n.indexOf("_uiType")||
(n=(n||"")+"\x26_uiType\x3dmobile");if(g=moqui.componentCache.get(h))console.info("found cached component for path "+h+": "+JSON.stringify(g)),b(g);else{var p=h;a.renderModes&&0<=a.renderModes.indexOf(e)&&(p+="."+e);if(p.slice(-e.length)===e)return console.info("loadComponent vue "+p+(c?" id "+c:"")),a={type:"GET",url:p,error:moqui.handleLoadError,success:function(m,t,r){205===r.status?(m=r.getResponseHeader("X-Redirect-To"),moqui.webrootVue.setUrl(m)):(m||b(moqui.NotFound),t=0<=r.getResponseHeader("Cache-Control").indexOf("max-age"),
moqui.isString(m)&&0<m.length?(m=httpVueLoader.parse(m,p.substr(0,p.lastIndexOf("/")+1)),t&&moqui.componentCache.put(h,m),b(m)):b(moqui.NotFound))}},l&&!$.isEmptyObject(l)&&(a.type="POST",a.data=l),$.ajax(a);var q=h.slice(-d.length)===d;!q&&a.renderModes&&0<=a.renderModes.indexOf(d)&&(p+="."+d,q=!0);q||(p+="."+f);k&&0<k.length&&(p+="/"+k);n&&0<n.length&&(p+="?"+n);console.info("loadComponent "+p+(c?" id "+c:""));a={type:"GET",url:p,error:moqui.handleLoadError,success:function(m,t,r){if(205===r.status)m=
r.getResponseHeader("X-Redirect-To"),moqui.webrootVue.setUrl(m);else{m||b(moqui.NotFound);var v=(t=r.getResponseHeader("Cache-Control"))&&0<=t.indexOf("max-age");if(moqui.isString(m)&&0<m.length)if(q||"define("===m.slice(0,7)){console.info("loaded JS from "+p+(c?" id "+c:""));var u=eval(m);u.template?(v&&moqui.componentCache.put(h,u),b(u)):(m=(h.slice(-d.length)===d?h.slice(0,-d.length):h)+"."+f,$.ajax({type:"GET",url:m,error:moqui.handleLoadError,success:function(w){u.template=w;v&&moqui.componentCache.put(h,
u);b(u)}}))}else m=m.replace(/<script/g,"\x3cm-script").replace(/<\/script>/g,"\x3c/m-script\x3e").replace(/<link/g,"\x3cm-stylesheet"),console.info("loaded HTML template from "+p+(c?" id "+c:"")),m={template:"\x3cdiv"+(c&&0<c.length?' id\x3d"'+c+'"':"")+"\x3e"+m+"\x3c/div\x3e"},v&&moqui.componentCache.put(h,m),b(m);else moqui.isPlainObject(m)?m.screenUrl&&m.screenUrl.length?moqui.webrootVue.setUrl(m.screenUrl):m.redirectUrl&&m.redirectUrl.length&&window.location.replace(m.redirectUrl):b(moqui.NotFound)}}};
...
e.prop("disabled",!0),setTimeout(function(){e.prop("disabled",!1)},3E3));var f=Object.keys(this.fields).length?new FormData:new FormData(this.$refs.qForm.$el);$.each(this.fields,function(l,p){f.set(l,p||"")});b=[];for(var g=f.entries()[Symbol.iterator]();;){var h=g.next();if(h.done)break;var n=h.value;h=n[0];n=n[1];moqui.isString(n)&&n.startsWith("__")&&f.set(h,"");!this.excludeEmptyFields||n&&n.length||b.push(h)}for(g=0;g<b.length;g++)f["delete"](b[g]);f.set("moquiSessionToken",this.$root.moquiSessionToken);
c&&f.set(c,d);this.parentCheckboxSet&&this.parentCheckboxSet.addCheckboxParameters&&this.parentCheckboxSet.addCheckboxParameters(f);this.$root.loading++;var k=new XMLHttpRequest;k.open(this.method,this.$root.appRootPath+this.action,!0);k.responseType="blob";k.withCredentials=!0;k.onload=function(){if(200===this.status){a.$root.loading--;var l=k.getResponseHeader("Content-Disposition");if(!l||-1===l.indexOf("attachment")&&-1===l.indexOf("inline")){var p=new FileReader;p.onload=function(r){r=r.target.result;
try{a.handleResponse(JSON.parse(r))}catch(v){a.handleResponse(r)}};p.readAsText(this.response)}else{var q=this.response;p="";l&&-1!==l.indexOf("attachment")&&(l=/filename[^;=\n]*=((['"]).*?\2|[^;\n]*)/.exec(l),null!=l&&l[1]&&(p=l[1].replace(/['"]/g,"")));if("undefined"!==typeof window.navigator.msSaveBlob)window.navigator.msSaveBlob(q,p);else{var m=window.URL||window.webkitURL,t=m.createObjectURL(q);p?(q=document.createElement("a"),"undefined"===typeof q.download?window.location.href=t:(q.href=t,
q.download=p,document.body.appendChild(q),q.click())):window.location.href=t;setTimeout(function(){m.revokeObjectURL(t)},100)}}}else moqui.handleLoadError(this,this.statusText,"")};k.setRequestHeader("Accept","application/json");k.send(f)},handleResponse:function(a){var b=!1;a&&moqui.isPlainObject(a)?(b=moqui.notifyMessages(a.messageInfos,a.errors),a.screenUrl&&0<a.screenUrl.length?this.$root.setUrl(a.screenUrl):a.redirectUrl&&0<a.redirectUrl.length&&(window.location.href=a.redirectUrl)):console.warn("m-form no response or non-JSON response: "+
...
q.startsWith("__")||d.set(p,q)});for(var e=[],f=[],g="",h=null,n=d.entries()[Symbol.iterator]();;){var k=n.next();if(k.done)break;var l=k.value;k=l[0];l=l[1];0===l.trim().length||"moquiSessionToken"===k||"moquiFormName"===k||0<k.indexOf("[]")||(0<k.indexOf("_op")||0<k.indexOf("_not")||0<k.indexOf("_ic")?e.push({name:k,value:l}):(f.push(k),this.bodyParameterNames&&0<=this.bodyParameterNames.indexOf(k)?(h||(h={}),h[k]=l):(0<g.length&&(g+="\x26"),g+=encodeURIComponent(k)+"\x3d"+encodeURIComponent(l))))}for(n=
0;n<e.length;n++)k=e[n],l=k.name.substring(0,k.name.indexOf("_")),0<=f.indexOf(l)&&(0<g.length&&(g+="\x26"),g+=encodeURIComponent(k.name)+"\x3d"+encodeURIComponent(k.value));a&&b&&b.trim().length&&(0<g.length&&(g+="\x26"),g+=encodeURIComponent(a)+"\x3d"+encodeURIComponent(b));a=this.action;a=0<a.indexOf("?")?a+"\x26"+g:a+"?"+g;this.$root.setUrl(a,h)},resetForm:function(){this.fields=Object.assign({},this.fieldsInitial)},clearForm:function(){this.fields={}}},mounted:function(){var a=$(this.$el);this.focusField&&
...
b.length;d++){var e=b[d];e!==a&&c.push(e)}b.length!==c.length&&this.$emit("input",c)}else console.warn("Tried to remove value from m-drop-down multiple "+this.name+" but value is not an Array")},clearAll:function(){this.$emit("input",null)}},mounted:function(){this.serverSearch&&(this.optionsUrl||console.error("m-drop-down in form "+this.form+" has no options-url but has server-search\x3dtrue"));if(this.optionsUrl&&this.optionsUrl.length){var a=this.dependsOn,b;for(b in a)if(a.hasOwnProperty(b)&&
this.fields){var c=this;this.$watch("fields."+a[b],function(){c.$emit("input",null);c.populateFromUrl({term:c.lastSearch})})}this.optionsLoadInit&&(this.serverSearch?this.value&&this.value.length&&moqui.isString(this.value)&&this.populateFromUrl({term:this.value}):this.populateFromUrl())}this.multiple||this.allowEmpty||this.value&&this.value.length||!this.options||!this.options.length||this.requiredManualSelect&&1!==this.options.length||this.$emit("input",this.options[0].value)}});
...
{},fullPath:this.currentLinkUrl,matched:[]}},setParameters:function(a){a&&(this.$root.currentParameters=$.extend({},this.$root.currentParameters,a));this.$root.reloadSubscreens()},addSubscreen:function(a){a.pathIndex=this.activeSubscreens.length;a.loadActive();this.activeSubscreens.push(a)},reloadSubscreens:function(){var a=this.currentPathList,b=this.activeSubscreens;console.info("reloadSubscreens currentPathList "+JSON.stringify(this.currentPathList));if(0===a.length&&0<b.length)b.splice(1),b[0].loadActive();
else for(var c=0;c<b.length&&!(c>=a.length);c++)b[c].loadActive()&&b.splice(c+1)},goPreviousScreen:function(){for(var a=this.currentPath,b=this.navHistoryList,c,d=0;d<b.length;d++)if(0>b[d].pathWithParams.indexOf(a)){c=b[d];break}c&&c.pathWithParams&&c.pathWithParams.length&&this.setUrl(c.pathWithParams)},addContainer:function(a,b){this.activeContainers[a]=b},reloadContainer:function(a){var b=this.activeContainers[a];b?b.reload():console.error("Container with ID "+a+" not found, not reloading")},
loadContainer:function(a,b){var c=this.activeContainers[a];c?c.load(b):console.error("Container with ID "+a+" not found, not loading url "+b)},addNavPlugin:function(a){var b=this;moqui.loadComponent(this.appRootPath+a,function(c){b.navPlugins.push(c)})},addNavPluginsWait:function(a,b){if(a&&a.length>b){this.addNavPlugin(a[b]);var c=this;a.length>b+1&&setTimeout(function(){c.addNavPluginsWait(a,b+1)},500)}},addAccountPlugin:function(a){var b=this;moqui.loadComponent(this.appRootPath+a,function(c){b.accountPlugins.push(c)})},
addAccountPluginsWait:function(a,b){if(a&&a.length>b){this.addAccountPlugin(a[b]);var c=this;a.length>b+1&&setTimeout(function(){c.addAccountPluginsWait(a,b+1)},500)}},addUrlListener:function(a){0<=this.urlListeners.indexOf(a)||this.urlListeners.push(a)},addNotify:function(a,b){var c=this.notifyHistoryList.slice(0),d=new Date,e=d.getHours();10>e&&(e="0"+e);d=d.getMinutes();10>d&&(d="0"+d);for(c.unshift({message:a,type:b,time:e+":"+d});25<c.length;)c.pop();this.notifyHistoryList=c},switchDarkLight:function(){this.$q.dark.toggle();
$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"QUASAR_DARK",preferenceValue:this.$q.dark.isActive?"true":"false"}})},toggleLeftOpen:function(){this.leftOpen=!this.leftOpen;$.ajax({type:"POST",url:this.appRootPath+"/apps/setPreference",error:moqui.handleAjaxError,data:{moquiSessionToken:this.moquiSessionToken,preferenceKey:"QUASAR_LEFT_OPEN",preferenceValue:this.leftOpen?"true":"false"}})},stopProp:function(a){a.stopPropagation()},
getNavHref:function(a){a||(a=this.navMenuList.length-1);a=this.navMenuList[a];if(a.extraPathList&&a.extraPathList.length){var b=a.path+"/"+a.extraPathList.join("/"),c=a.pathWithParams.indexOf("?");0<c&&(b+=a.pathWithParams.slice(c));return b}return a.pathWithParams||a.path},getLinkPath:function(a){moqui.isPlainObject(a)&&(a=moqui.makeHref(a));this.appRootPath&&this.appRootPath.length&&0!==a.indexOf(this.appRootPath)&&(a=this.appRootPath+a);a=a.split("/");var b=this.appRootPath.split("/").length;a[b]=
this.linkBasePath.split("/").slice(-1);return a=a.join("/")},getQuasarColor:function(a){return moqui.getQuasarColor(a)}},watch:{navMenuList:function(a){if(0<a.length){var b=a[a.length-1];a=1<a.length?a[a.length-2]:null;b.extraPathList&&(this.extraPathList=b.extraPathList);var c=this.basePathSize,d=b.path.split("/").slice(c+1);console.info("nav updated fullPath "+JSON.stringify(d)+" currentPathList "+JSON.stringify(this.currentPathList)+" cur.path "+b.path+" basePathSize "+c);this.currentPathList=
d;this.reloadSubscreens();a=(a?a.title+" - ":"")+b.title;c=b.pathWithParams;var e=c.indexOf("?");if(0<e){d=["pageIndex","orderBySelect","orderByField","moquiSessionToken"];var f=c.substring(e+1).split("\x26");c=c.substring(0,e);e=0;for(var g="",h=0;h<f.length;h++){var n=f[h];c=-1===c.indexOf("?")?c+"?":c+"\x26";c+=n;if(!(3<e)){var k=n.indexOf("\x3d");if(0<k){var l=n.substring(0,k);n=n.substring(k+1);0<l.indexOf("_op")||0<l.indexOf("_not")||0<l.indexOf("_ic")||0<d.indexOf(l)||l===n||(0<g.length&&(g+=
", "),g+=decodeURIComponent(n),e++)}}}0<g.length&&(70<g.length&&(g=g.substring(0,70)+"..."),a=a+" ("+g+")")}d=this.navHistoryList;for(f=0;f<d.length;)d[f].pathWithParams===c?d.splice(f,1):f++;for(d.unshift({title:a,pathWithParams:c,image:b.image,imageType:b.imageType});25<d.length;)d.pop();document.title=a}},currentPathList:function(a){(a=a[a.length-1])&&$(this.$el).removeClass().addClass(a)}},computed:{currentPath:{get:function(){var a=this.currentPathList,b=this.extraPathList;return this.basePath+
(a&&0<a.length?"/"+a.join("/"):"")+(b&&0<b.length?"/"+b.join("/"):"")},set:function(a){a&&0!==a.length?("/"===a.slice(a.length-1)&&(a=a.slice(0,a.length-1)),0===a.indexOf(this.linkBasePath)?a=a.slice(this.linkBasePath.length+1):0===a.indexOf(this.basePath)&&(a=a.slice(this.basePath.length+1)),this.currentPathList=a.split("/")):this.currentPathList=[]}},currentLinkPath:function(){var a=this.currentPathList,b=this.extraPathList;return this.linkBasePath+(a&&0<a.length?"/"+a.join("/"):"")+(b&&0<b.length?
"/"+b.join("/"):"")},currentSearch:{get:function(){return moqui.objToSearch(this.currentParameters)},set:function(a){this.currentParameters=moqui.searchToObj(a)}},currentLinkUrl:function(){var a=this.currentSearch;return this.currentLinkPath+(0<a.length?"?"+a:"")},basePathSize:function(){return this.basePath.split("/").length-this.appRootPath.split("/").length},ScreenTitle:function(){return 0<this.navMenuList.length?this.navMenuList[this.navMenuList.length-1].title:""},documentMenuList:function(){for(var a=
[],b=0;b<this.navMenuList.length;b++){var c=this.navMenuList[b].screenDocList;c&&c.length&&c.forEach(function(d){a.push(d)})}return a}},created:function(){this.moquiSessionToken=$("#confMoquiSessionToken").val();this.appHost=$("#confAppHost").val();this.appRootPath=$("#confAppRootPath").val();this.basePath=$("#confBasePath").val();this.linkBasePath=$("#confLinkBasePath").val();this.userId=$("#confUserId").val();this.locale=$("#confLocale").val();moqui.localeMap[this.locale]&&(this.locale=moqui.localeMap[this.locale]);
this.leftOpen="true"===$("#confLeftOpen").val();var a=$("#confDarkMode").val();this.$q.dark.set("true"===a);this.notificationClient=new moqui.NotificationClient(("https:"===location.protocol?"wss://":"ws://")+this.appHost+this.appRootPath+"/notws");var b=[];$(".confNavPluginUrl").each(function(d,e){b.push($(e).val())});this.addNavPluginsWait(b,0);var c=[];$(".confAccountPluginUrl").each(function(d,e){c.push($(e).val())});this.addAccountPluginsWait(c,0)},mounted:function(){$(this.$el).css("display",
"initial");this.setUrl(window.location.pathname+window.location.search);this.notificationClient.registerListener("ALL");window.Notification&&"granted"!==Notification.permission&&"denied"!==Notification.permission&&Notification.requestPermission(function(a){"granted"===a?moqui.notifyMessages("Browser notifications enabled, if you don't want them use browser notification settings to block"):"denied"===a&&moqui.notifyMessages("Browser notifications disabled, if you want them use browser notification settings to allow")})}});
