...
| 8 | Lesson content is not gated by subscription: anyone in the tenant can fetch a full course through `get#Course` | `get#Course` | Paid courses |
...
- Check the learner's subscription in `get#Course` before returning lesson content (the owner/admin keeps full access).
