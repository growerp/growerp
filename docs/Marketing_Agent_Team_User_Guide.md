...
* Write policy: **approve** — every publish waits for you in **Agent Control → Approvals**
...
* **Agent Control → Approvals** — approve or reject the social posts *Content and Social*
  queued for publishing. Nothing is published until you approve it.
...
* Publishing to external platforms is **approval-gated**.
* No agent sends messages to prospects on its own; the send scheduler (a separate job) drains
  PENDING outreach messages within your daily limits.
...
| Social posts never go out | They wait in **Approvals** until you approve them. |
