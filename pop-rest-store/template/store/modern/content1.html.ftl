<#-- url prefix of a translated site, '' for the language the site is written in;
     the sidebar links have to stay inside the language being served -->
<#assign up = urlPrefix!''>
        ...
        <aside class="lg:w-72 shrink-0">
            <div class="l-glass rounded-2xl p-6 lg:sticky lg:top-24">
                <h5 class="font-display font-semibold text-on-surface mb-4 flex items-center gap-2">
                    <span class="material-symbols-outlined text-primary text-[20px]">menu_book</span>${l('GrowerpWebsiteNavigation')}
                </h5>
<#list storeInfo.menu1 as topMenu>
    <#if topMenu.title != 'obsidian'>
                <div class="mb-4">
                    <a href="${up}/content/${topMenu.path}#${topMenu.anchor}" class="flex items-center gap-2 font-label text-sm font-medium text-on-surface hover:text-primary transition-colors">
                    ...
                    <#list topMenu.items as item>
                        <li>
                            <a href="${up}/content/${topMenu.path}#${item.anchor}" class="flex items-center gap-2 text-sm text-on-surface-variant hover:text-primary transition-colors">
...
<#macro showPage page>
    <li>
        <a href="${up}/content/${page.path}" class="flex items-center gap-2 text-sm text-on-surface-variant hover:text-primary transition-colors">
