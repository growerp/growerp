...
echo "Building GrowERP Rental snap package..."

# Ensure we're in the correct directory
cd "$(dirname "$0")"

# Check if Flutter Linux build exists
if [ ! -f "build/linux/x64/release/bundle/rental" ]; then
...
SNAP_FILE=$(ls growerp-rental_*.snap 2>/dev/null | head -1)
if [ -n "$SNAP_FILE" ]; then
    ...
    echo ""
    echo "To uninstall:"
    echo "sudo snap remove growerp-rental"
    echo ""
    ...
    echo "sudo snap install growerp-rental --beta|stable|edge"
    echo ""
    echo "To remove to change version:"
    echo "sudo snap remove growerp-rental [--force]"
    echo ""
else
