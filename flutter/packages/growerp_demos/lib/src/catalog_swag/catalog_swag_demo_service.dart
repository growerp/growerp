...
/// Shared progress helper for the catalog-swag demo.
/// Key prefix matches the old DemoProgressService keys for backward compat.
final catalogSwagProgress = DemoProgressHelper('catalog_swag_demo');
...
Future<String> setupDemoData(
  ...
  final bomExists =
      boms.boms.any((b) => b.productPseudoId == _swagPackage.pseudoId);
...
Future<String> createSalesOrder(
  ...
  if (pkg == null) {
    throw Exception('SWAG-PKG-001 product not found. Run Setup first.');
  }
  ...
  if (customer == null) {
    throw Exception('Demo customer not found. Run Setup first.');
  }
  ...
  await catalogSwagProgress.saveSalesOrderId(placed.orderId ?? '', ownerPartyId);
...
Future<String> createAndApprovePurchaseOrder(
  ...
  if (supplier == null) {
    throw Exception('Demo supplier not found. Run Setup first.');
  }
  ...
  await catalogSwagProgress.savePurchaseOrderId(
    approved.orderId ?? '',
    ownerPartyId,
  );
...
Future<String> receiveIncomingShipment(
  ...
  if (shipment == null) {
    throw Exception(
      'No incoming shipment found. Approve the purchase order first.',
    );
  }
...
Future<String> completeWorkOrder(
  ...
  WorkOrders wos =
      await rc.getWorkOrder(search: _swagPackage.pseudoId, limit: 5);
  ...
  if (wo == null) {
    throw Exception(
      'No active work order found for Moqui Marketing Package. '
...
Future<String> updateStatsAndLedger(
  ...
  return 'Ledger totals and statistics updated. '
