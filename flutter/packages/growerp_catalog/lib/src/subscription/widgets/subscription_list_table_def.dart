...
TableData getSubscriptionTableData(
  Bloc bloc,
  ...
  var fromDate = Text(
    item.fromDate.toLocalizedDateOnly(context),
    key: Key('fromDate$index'),
    textAlign: TextAlign.left,
  );
  var thruDate = Text(
    item.thruDate.toLocalizedDateOnly(context),
  ...
  if (isPhone) {
    ...
    rowContent.add(
      TableRowContent(name: 'From Date', width: 8, value: fromDate),
    );

    rowContent.add(
      TableRowContent(name: 'Thru Date', width: 8, value: thruDate),
    );
  }
  if (!isPhone) {
    rowContent.add(
      TableRowContent(
        ...
        width: 8,
        value: Text(
          item.purchaseFromDate.toLocalizedDateOnly(context),
          textAlign: TextAlign.left,
        ),
      ),
    );
  ...
  if (!isPhone) {
    rowContent.add(
      TableRowContent(
        ...
        width: 8,
        value: Text(
          item.purchaseThruDate.toLocalizedDateOnly(context),
          textAlign: TextAlign.left,
        ),
      ),
    );
