...
class RequestTest {
  ...
  static Future<void> addRequests(WidgetTester tester, List<FinDoc> requests,
  ...
  static Future<void> updateRequests(
      WidgetTester tester, List<FinDoc> requests) async {
    SaveTest test = await PersistFunctions.getTest();
  ...
  static Future<List<FinDoc>> enterRequestData(
    ...
    for (FinDoc request in requests) {
      ...
      await CommonTest.enterDropDownSearch(tester, 'otherCompanyUser',
  ...
  static Future<void> checkRequest(
    ...
    for (FinDoc request in requests) {
      await CommonTest.doNewSearch(tester,
          searchString: request.requestId!, seconds: CommonTest.waitTime);
      expect(
