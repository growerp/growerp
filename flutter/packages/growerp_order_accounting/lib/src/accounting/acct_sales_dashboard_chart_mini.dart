...
/// Compact accounts-receivable dashboard for the half-height 'Acct Sales'
/// dashboard tile: dense invoice-status funnel bars (stage + count only)
/// with the invoice counters in a row at the bottom. The tile route must be
/// listed in DashboardGrid.compactGraphicRoutes so the icon+title render
/// beside it.
class AcctSalesDashboardChartMini extends StatefulWidget {
  const AcctSalesDashboardChartMini({super.key});

  @override
  State<AcctSalesDashboardChartMini> createState() =>
      _AcctSalesDashboardChartMiniState();
}

class _AcctSalesDashboardChartMiniState
    extends State<AcctSalesDashboardChartMini> {
  AcctSalesDashboard? dashboard;
  String? error;
  ...
  Future<void> _load() async {
    try {
      final result = await context.read<RestClient>().getAcctSalesDashboard();
  ...
  Widget _funnel(
    ...
    List<AcctSalesStageSummaryItem> summary,
  ) {
              ...
              child: Row(
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      '${item.count}',
                      key: Key('acctSalesStage${item.stageId}'),
                      style: labelStyle,
                    ),
                  ),
                ],
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        key: const Key('acctSalesDashboardMini'),
        children: [
