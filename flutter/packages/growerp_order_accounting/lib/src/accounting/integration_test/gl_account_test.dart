...
class GlAccountTest {
  ...
  static Future<void> updateGlAccounts(
      WidgetTester tester, List<GlAccount> glAccounts) async {
    SaveTest test = await PersistFunctions.getTest();
    ...
    if (newGlAccounts[0].accountName != glAccounts[0].accountName) {
      ...
      for (int x = 0; x < test.glAccounts.length; x++) {
        newGlAccounts[x] =
            glAccounts[x].copyWith(glAccountId: test.glAccounts[x].glAccountId);
      }
  ...
  static Future<List<GlAccount>> checkGlAccount(
    ...
    for (GlAccount glAccount in glAccounts) {
      ...
      expect(
          CommonTest.getTextFormField('code'), equals(glAccount.accountCode!));
      expect(
          CommonTest.getTextFormField('name'), equals(glAccount.accountName!));
      if (glAccount.accountType != null) {
