...
echo "Building GrowERP Freelance snap package..."

# Ensure we're in the correct directory
cd "$(dirname "$0")"

# Check if Flutter Linux build exists
if [ ! -f "build/linux/x64/release/bundle/freelance" ]; then
...
SNAP_FILE=$(ls growerp-freelance_*.snap 2>/dev/null | head -1)
if [ -n "$SNAP_FILE" ]; then
    ...
    echo "growerp-freelance"
    echo ""
    echo "To uninstall:"
    echo "sudo snap remove growerp-freelance"
    echo ""
    ...
    echo "sudo snap install growerp-freelance --beta|stable|edge"
    echo ""
    echo "To remove to change version:"
    echo "sudo snap remove growerp-freelance [--force]"
    echo ""
else
