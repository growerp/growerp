...
import 'package:freelance/views/freelance_db_form.dart';
...
const _freelanceMenuConfig = MenuConfiguration(
  menuConfigurationId: 'FREELANCE_DEFAULT',
  appId: 'freelance',
  name: 'Freelance',
  menuItems: [],
);

GoRouter createFreelanceScreenshotRouter() {
  WidgetRegistry.clear();
  for (final reg in freelanceWidgetRegistrations) {
    WidgetRegistry.register(reg);
  }
  return createDynamicAppRouter(
    [_freelanceMenuConfig],
...
Future<String> _resolveScreenshotPath(String name) async {
  ...
  try {
    ...
    final fallbackDir = Directory(
      '${Directory.systemTemp.path}/growerp_screenshots/freelance',
    );
...
void main() {
  ...
  const applicationId = 'AppFreelance';
  ...
  testWidgets('freelance store screenshots', (WidgetTester tester) async {
    final restClient = RestClient(await buildDioClient());

    await CommonTest.startTestApp(
      tester,
      createFreelanceScreenshotRouter(),
      _freelanceMenuConfig,
      delegates,
      restClient: restClient,
      blocProviders: getFreelanceBlocProviders(restClient, applicationId),
      applicationId: applicationId,
      clear: true,
      title: 'Freelance Screenshots',
    );

    await CommonTest.createCompanyAndAdmin(tester, demoData: true);

    await _waitForAnyKey(tester, ['refresh', 'tap/tasks']);
