...
const _supportMenuConfig = MenuConfiguration(
...
GoRouter createSupportScreenshotRouter() {
...
Future<String> _resolveScreenshotPath(String name) async {
  ...
  try {
    ...
    final fallbackDir = Directory(
      '${Directory.systemTemp.path}/growerp_screenshots/support',
    );
...
void main() {
  ...
  testWidgets('support store screenshots', (WidgetTester tester) async {
    final restClient = RestClient(await buildDioClient());

    await CommonTest.startTestApp(
      tester,
      createSupportScreenshotRouter(),
      _supportMenuConfig,
      delegates,
    ...
    await _selectOption(
      tester,
      route: '/applications',
      formKey: '/applications',
    );
    await _screenshot(binding, tester, 'applications');

    await _selectOption(tester, route: '/owners', formKey: '/owners');
    await _screenshot(binding, tester, 'owners');

    await _selectOption(
      tester,
      route: '/restStatistics',
      formKey: '/restStatistics',
    );
    await _screenshot(binding, tester, 'statistics');
  });
}
