...
/// Compact manufacturing dashboard for the half-height 'Manufacturing'
/// dashboard tile: dense work-order status funnel bars (stage + count only)
/// with the work order counters in a row at the bottom. The tile route
/// must be listed in DashboardGrid.compactGraphicRoutes so the icon+title
/// render beside it.
class ManufacturingDashboardChartMini extends StatefulWidget {
  const ManufacturingDashboardChartMini({super.key});

  @override
  State<ManufacturingDashboardChartMini> createState() =>
      _ManufacturingDashboardChartMiniState();
}

class _ManufacturingDashboardChartMiniState
    extends State<ManufacturingDashboardChartMini> {
  ManufacturingDashboard? dashboard;
  String? error;
  ...
  Widget _funnel(
    ...
    List<ManufacturingStageSummaryItem> summary,
  ) {
    final colorScheme = Theme.of(context).colorScheme;
    if (summary.isEmpty) {
      return Center(child: Text(ManufacturingLocalizations.of(context)!.noWorkOrderData));
    }
    int maxCount = 1;
              ...
              child: Row(
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      '${item.count}',
                      key: Key('manufacturingStage${item.stageId}'),
                      style: labelStyle,
                    ),
                  ),
                ],
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        key: const Key('manufacturingDashboardMini'),
        children: [
