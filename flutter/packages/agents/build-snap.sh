...
echo "Building GrowERP Agents snap package..."
...
SNAP_FILE=$(ls growerp-agents_*.snap 2>/dev/null | head -1)
if [ -n "$SNAP_FILE" ]; then
    ...
    echo ""
    echo "To uninstall:"
    echo "sudo snap remove growerp-agents"
    echo ""
    ...
    echo "sudo snap install growerp-agents --beta|stable|edge"
    echo ""
    echo "To remove to change version:"
    echo "sudo snap remove growerp-agents [--force]"
    echo ""
else
