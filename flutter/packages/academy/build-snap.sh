...
echo "Building GrowERP Academy snap package..."

# Ensure we're in the correct directory
cd "$(dirname "$0")"

# Check if Flutter Linux build exists
if [ ! -f "build/linux/x64/release/bundle/academy" ]; then
...
SNAP_FILE=$(ls growerp-academy_*.snap 2>/dev/null | head -1)
if [ -n "$SNAP_FILE" ]; then
    ...
    echo "growerp-academy"
    echo ""
    echo "To uninstall:"
    echo "sudo snap remove growerp-academy"
    echo ""
    ...
    echo "sudo snap install growerp-academy --beta|stable|edge"
    echo ""
    echo "To remove to change version:"
    echo "sudo snap remove growerp-academy [--force]"
    echo ""
else
