...
echo "Building GrowERP Marketing snap package..."

# Ensure we're in the correct directory
cd "$(dirname "$0")"

# Check if Flutter Linux build exists
if [ ! -f "build/linux/x64/release/bundle/marketing" ]; then
...
SNAP_FILE=$(ls growerp-marketing_*.snap 2>/dev/null | head -1)
if [ -n "$SNAP_FILE" ]; then
    ...
    echo "growerp-marketing"
    echo ""
    echo "To uninstall:"
    echo "sudo snap remove growerp-marketing"
    echo ""
    ...
    echo "sudo snap install growerp-marketing --beta|stable|edge"
    echo ""
    echo "To remove to change version:"
    echo "sudo snap remove growerp-marketing [--force]"
    echo ""
else
