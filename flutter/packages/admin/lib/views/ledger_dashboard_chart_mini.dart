...
/// posting/chart-of-accounts counters in a row at the bottom. The tile
/// route must be listed in DashboardGrid.compactGraphicRoutes so the
/// icon+title render beside it.
class LedgerDashboardChartMini extends StatefulWidget {
  const LedgerDashboardChartMini({super.key});

  @override
  State<LedgerDashboardChartMini> createState() =>
      _LedgerDashboardChartMiniState();
}

class _LedgerDashboardChartMiniState extends State<LedgerDashboardChartMini> {
  LedgerDashboard? dashboard;
  String? error;
  ...
  Future<void> _load() async {
    try {
      final result = await context.read<RestClient>().getLedgerDashboard();
              ...
              child: Row(
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      '${item.count}',
                      key: Key('ledgerStage${item.stageId}'),
                      style: labelStyle,
                    ),
                  ),
                ],
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        key: const Key('ledgerDashboardMini'),
        children: [
