...
/// Compact inventory dashboard for the half-height 'Inventory' dashboard
/// tile: dense shipment-fulfillment funnel bars (stage + count only) with
/// the warehouse counters in a row at the bottom. The tile route must be
/// listed in DashboardGrid.compactGraphicRoutes so the icon+title render
/// beside it.
class InventoryDashboardChartMini extends StatefulWidget {
  const InventoryDashboardChartMini({super.key});

  @override
  State<InventoryDashboardChartMini> createState() =>
      _InventoryDashboardChartMiniState();
}

class _InventoryDashboardChartMiniState
    extends State<InventoryDashboardChartMini> {
  ...
  Future<void> _load() async {
    try {
      final result = await context.read<RestClient>().getInventoryDashboard();
  ...
  Widget _funnel(
    ...
    List<InventoryStageSummaryItem> summary,
  ) {
              ...
              child: Row(
                children: [
                  ...
                  Padding(
                    ...
                    child: Text(
                      '${item.count}',
                      key: Key('inventoryStage${item.stageId}'),
                      style: labelStyle,
                    ),
                  ),
                ],
  ...
  Widget build(BuildContext context) {
    ...
    return Padding(
      ...
      child: Column(
        key: const Key('inventoryDashboardMini'),
        children: [
