...
import 'package:growerp_marketing/src/social_post/integration_test/content_calendar_test.dart';
...
void main() {
  ...
  testWidgets('''GrowERP content calendar test''', (tester) async {
    ...
    await CommonTest.startTestApp(
      ...
      title: 'GrowERP content calendar test',
      clear: true,
    );
    ...
    await SocialPostTest.addSocialPosts(
      tester,
      marketing_data.socialPosts.sublist(0, 1),
    );

    // today is always inside the calendar's default month period
    final DateTime scheduled = DateTime.now();
    await ContentCalendarTest.scheduleFirstPost(tester, scheduled);

    await ContentCalendarTest.selectContentCalendar(tester);
    await ContentCalendarTest.checkPostOnDay(tester, scheduled);
